...
public class FancyClock extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        drawCircle(140, Color.GREY, Color.WHITE, root);
    ...
    /**
     * Draws numbers one of the values from 1 to 12
     * @param value value of the numbers from 1 to 12
     * @param root root of the Pane
     */
    public static void drawNumbers(int value, Pane root){
        double angle = (value - 3)*Math.PI/6;
        double px = CX + (RADIUS-50)*Math.cos(angle);
        double py = CY + (RADIUS-50)*Math.sin(angle);
        String temp = Integer.toString(value);
        drawText(px ,py, temp, 20, root);
    }
